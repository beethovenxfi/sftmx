...
contract FTMStakingV1_1 is Initializable, OwnableUpgradeable, UUPSUpgradeable {
    ...
    function pickVaultsToUndelegate(uint256 amount)
        public
        view
        returns (UndelegateInfo[] memory, uint256)
    {
    ...
    function _currentStakeValue(address payable vault)
        internal
        view
        returns (uint256)
    {
        ...
        uint256 rewardsReal = rewardsAll -
            (rewardsAll * protocolFeeBIPS) /
            10_000;
        ...
        if (isSlashed) {
            penalty = _getSlashingPenalty(
                stake + matured,
                SFC.slashingRefundRatio(toValidatorID)
            );
        }
        return stake + rewardsReal + matured - penalty;
    }

    function _getSlashingPenalty(uint256 amount, uint256 refundRatio)
        internal
        pure
        returns (uint256)
    {
    ...
    function _incrementVaultPtr() internal {
        uint256 candidatePtr = _incrementWithMod(
            currentVaultPtr,
            maxVaultCount
        );
