...
import FTMStakingV0_2Abi from '../artifacts/contracts/FTMStakingV0_2.sol/FTMStaking.json'
...
describe('Test ftm staking contract', function () {
    ...
    it('deploy new contract', async () => {
        ...
        const FTMStakingV0_2 = await ethers.getContractFactory('contracts\\FTMStakingV0_2.sol:FTMStaking', {
            libraries: {
                SFCPenalty: sfcPenalty.address,
            },
        })
        console.log('Deploying FTMStakingV0_2')
        const ftmStakingV0_2 = await FTMStakingV0_2.deploy()
        console.log('Deployed FTMStakingV0_2', ftmStakingV0_2.address)
    })

    it('upgrade proxy', async () => {
        const ftmStaking = await ethers.getContractAt(FTMStakingV0_2Abi.abi, FTM_STAKING_PROXY)
        ...
        console.log('Upgrading FTMStakingV0_2')
        await ftmStaking.connect(treasurySigner).upgradeTo('0xab16A69A5a8c12C732e0DEFF4BE56A70bb64c926')
        console.log('FTMStakingV0_2 upgraded')
    ...
    it('pause penalty', async () => {
        const ftmStaking = await ethers.getContractAt(FTMStakingV0_2Abi.abi, FTM_STAKING_PROXY)

        const penaltyPausedBefore = await ftmStaking.penaltyPaused()
        expect(penaltyPausedBefore).to.be.false
        const pauseTxn = await ftmStaking.connect(treasurySigner).setPenaltyPaused(true, {
            gasPrice: '0x0',
        })
        await pauseTxn.wait()
        const penaltyPausedAfter = await ftmStaking.penaltyPaused()
        expect(penaltyPausedAfter).to.be.true
    })

    it('withdraw a lot of sftmx without penalty applied', async () => {
        const sftmx = await ethers.getContractAt('ERC20', SFTMX)
        const staking = await ethers.getContractAt(FTMStakingV0_2Abi.abi, FTM_STAKING_PROXY)
        ...
        const sftmxToUndelegate = parseEther('1000000')
        ...
        const withdrawalPenalty = withdrawalRequest.penalty

        expect(withdrawalFromPoolAmount).to.be.gt(0)
        expect(withdrawalFromUndelegateAmount).to.be.gt(0)
        expect(withdrawalPenalty).to.be.eq(0)
