...
async function main() {
    ...
    const maxVaultCount = await ftmStaking.maxVaultCount({ blockTag: blockNumber })
    const exchangeRate = await ftmStaking.getExchangeRate({ blockTag: blockNumber })
    console.log(maxVaultCount.toString())

    let totalStaked = BigNumber.from('0')

    for (let i = 0; i < maxVaultCount; i++) {
        const vaultAddress = await ftmStaking.getVault(i, { blockTag: blockNumber })
        if (vaultAddress !== '0x0000000000000000000000000000000000000000') {
            const vault = await ethers.getContractAt(Vault.abi, vaultAddress)
            const stakedAmount = await vault.currentStakeValue({ blockTag: blockNumber })
            const validatorId = await vault.toValidatorID({ blockTag: blockNumber })
            const owner = await vault.owner({ blockTag: blockNumber })
    ...
    const maturedVaultCount = await ftmStaking.getMaturedVaultLength({ blockTag: blockNumber })
    let totalMatured = BigNumber.from('0')

    for (let i = 0; i < maturedVaultCount; i++) {
        const vaultAddress = await ftmStaking.getMaturedVault(i, { blockTag: blockNumber })
        if (vaultAddress !== '0x0000000000000000000000000000000000000000') {
            const vault = await ethers.getContractAt(Vault.abi, vaultAddress)
            const stakedAmount = await vault.currentStakeValue({ blockTag: blockNumber })
            const validatorId = await vault.toValidatorID({ blockTag: blockNumber })
    ...
    const ftmPoolAmount = await ftmStaking.getPoolBalance({ blockTag: blockNumber })
